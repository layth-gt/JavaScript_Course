...
function render () { 
    //create markup by rducing the whloe object
    var markup = glossary.reduce (function(sum,index){
        //create the markup for the tags because it's an array and needs it's own reduce
        var allTags = index.tags;
        var tagsMarkup = allTags.reduce(function (currentHTML, tag){
            //returning the markup for tags
            return currentHTML + `${tag} `;
        },"");

        //returning the markup for the whole object combined with the tags array
        return ( 
            sum + `
            <h2>Term: ${index.term}</h2>
            <h3>Class: ${index.class}</h3>
            <p>${index.definition}</p>
            <div>Tags: <strong>${tagsMarkup}</strong></div>
            <hr>
    ...
    //adding the markup to the HTML body
...
//     //assigning the input to a var to check if it exist
//     var termToSearch = searchInput.value;

//     //running the search for what was input
//     searchTerms (termToSearch);

//     //flitering the searched term
//     function searchTerms (termToSearch) {
//         var allGlossaryTerms = glossary.filter(function(i) {
//             return i.term === termToSearch; 
//         });
        
//         var searchResultMarkup = allGlossaryTerms.reduce(function(resultHTML,index){
...
//         searchOutput.innerHTML = searchResultMarkup;
...
searchInput.addEventListener("keyup", hotSearch);

function hotSearch(keyPressed) {

  var hotSearchWord = searchInput.value.toUpperCase();
  
  searchTerms (hotSearchWord);
  console.log (hotSearchWord);
...
function searchTerms (termToSearch) {
          var allGlossaryTerms = glossary.filter(function(i) {
              ...
              return i.term.toLocaleUpperCase() === termToSearch; 
          });
...
classList.addEventListener("click", fliterByClass);

function fliterByClass () {
  var choosenClass = classList.value;
  var allClasses = glossary.filter(function(i){
    return i.class == choosenClass;

  },"");

  var classResulMarkup = allClasses.reduce(function(total,index){
    return (total + `
    <h2>Term: ${index.term}</h2>
            <h3>Class: ${index.class}</h3>
            <p>${index.definition}</p>
  ...
  searchOutput.innerHTML = classResulMarkup;
